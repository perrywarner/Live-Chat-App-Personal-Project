...
describe('Integration: User Service with Message Service', () => {
    ...
    test('when Message is created, it should have been sentBy a valid User', () => {
        ...
        const expected: Message[] = [ messageData[1], { ...mockSubmission, sentBy: existingUser!.name, createTime: Date.now() }]
    ...
    it.todo('after creating a Message, the corresponding User that the Message was sentBy should have sentMessages that contains the new Message')
})
